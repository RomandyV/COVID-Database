...
public class CovidDatabaseGUI extends JFrame implements ActionListener{
    ...
    public static void main(String args[]){
        ...
        gui.setTitle("Romandy Vu Covid Database");
        gui.pack();
        gui.setVisible(true);
    }
    ...
    private void displayPeakDeaths(){
        ...
        if(isValidState()){ 
            ...
            results.setText("\nPeak Daily Deaths for " + st.toUpperCase() + "\n");
        ...
        if(isValidMonth() && isValidDay()){
            ...
            c = database.peakDailyDeaths(m,d);
    ...
    private void displayDailyInfections(){
        if(isValidMonth() && isValidDay() && isValidMinInfections()){
            ...
            results.append("\nAll states with at least " + count 
    ...
    private void displaySafeToOpen(){
        ...
        if(isValidState()){
            ...
            if(database.safeToOpen(st) != null){
                displayRecords(list);
                results.append("It is safe to open for " + st +"\n");
            
            }
            else{
                results.append("It is not safe to open for " + st + "\n");
    ...
    private void displayTopTen(){
        ...
        if((isValidMonth()) && (isValidDay())){
            int m = Integer.parseInt(monthFld.getText());
            int d = Integer.parseInt(dayFld.getText());
            ArrayList <CovidEntry> list = database.topTenDeaths(m,d);
            results.append("Top 10 deaths on " + m + "/" + d + " are:\n");
            if (list.size() > 0) {
                displayRecords (list);
            }    
            else {
                results.append("\nNo records found for the date and minimum value");
            }
        
        }
        else{
            results.append("Invalid date\n");
    ...
    private boolean isValidDay(){
        ...
        try{
            ...
            isValid = false;
            JOptionPane.showMessageDialog(this, "Enter a valid month (1-31)");
        }
    ...
    private boolean isValidMinInfections(){
        ...
        try{
            ...
            if((isValidDay() == false) || (isValidMonth() == false) || (val - val != 0))
               throw new NumberFormatException("Not a valid input");
               
            
        }catch (NumberFormatException e){
            isValid = false;
            JOptionPane.showMessageDialog(this, "Enter a valid day (1-31), month (1-12), and a minimum value (an integer)");
        }
    ...
    private boolean isValidState(){
        ...
        try{
            String st = stateFld.getText();
            for(String states: database.listOfStates()){
            ...
            isValid = false;
            JOptionPane.showMessageDialog(this, "Enter a valid state abbreviation");
